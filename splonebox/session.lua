...
local MsgpackRpcStream = require('splonebox.msgpack_rpc_stream')
local CryptoStream = require("splonebox.crypto_stream")
...
function Session.new(stream, cstream)
  return setmetatable({
    _crypto_stream = cstream,
    _msgpack_rpc_stream = MsgpackRpcStream.new(stream, cstream),
    _pending_messages = {},
...
function Session:crypto_init()
  self._crypto_stream:init()
  self._crypto_stream:read_start(on_request, uv.stop)
  uv.run()
end
